...
class Symbol{
    geometry : any; // The probability that this Rule will be used when replacing a character in the grammar string
    position : vec3;
    scale : number;
    constructor(geometry: any, position: vec3, scale: number) {
    }
}
...
class Shape extends Drawable {
  ...
  constructor(geometry: any, position: vec3, scale: number) {
    ...
    //grammar random
    this.symbol = Symbol(geometry, position, scale);
